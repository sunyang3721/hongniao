...
class goods_sku_table extends table {
    ...
    public function spu(){
        ...
        if(isset($this->result['goods']['delivery_set_type'])){
            if($this->result['goods']['delivery_set_type'] == 1){
                unset($this->result['goods']['weight'],$this->result['goods']['volume'],$this->result['goods']['delivery_template_id']);
            }else{
                unset($this->result['sku']['weight'],$this->result['sku']['volume'],$this->result['sku']['delivery_template_id']);
            }
        }else{
