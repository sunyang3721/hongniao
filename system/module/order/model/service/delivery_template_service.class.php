...
class delivery_template_service extends service {
	...
	public function get_lists($sqlmap = array(), $page = 1, $limit = 10, $order = 'sort ASC', $field = ''){
		...
		foreach ($result as $k => $v) {
			...
			$info = json_decode($v['delivery_info'],true);
			$result[$k]['_first_price_'] = $info[0]['template']['first_fee'].$string;
			$result[$k]['_follow_price_'] = $info[0]['template']['follow_fee'].$string;
			$districts = explode(',', $info[0]['district_ids']);
			$_districts = array();
			foreach ($districts as $district) {
				$_districts[] = $this->load->table('admin/district')->where(array('id' => $district))->getField('name');
			}
			$result[$k]['_district_'] = implode(',', $_districts);
			unset($result[$k]['enabled']);
			$result[$k]['enabled'] = $v['enabled'];
		}
	...
	public function set_default($id) {
		...
		$this->table->where(array("id" => $id))->setField('enabled', 1);
	...
	public function status($id){
		...
		$info = $this->table->where(array('id' => $id))->find();
		if(!$info){
			$this->error = lang('_select_not_exist_');
			return FALSE;
		}
		if($info['isdefault'] == 1){
			$this->error = lang('default_delivery_template_cant_close','order/language');
			return FALSE;
		}
		$data = array();
		$data['enabled']=array('exp',' 1-enabled ');
