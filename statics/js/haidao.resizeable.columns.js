...
(function($, window) {
	...
	var resizeableColumns = (function(){
		...
		function resizeableColumns($table,options){
			...
			$table.after('<div class="layout table-get-width"></div>');
		...
		resizeableColumns.prototype.setLinesWidth = function(){
			...
			var tableWidth = $($table).next(".table-get-width").width();
			...
			$table.find(".resize-th").children(".th").each(function(){
				...
				if($(this).index()!=$table.find(".resize-th").children(".th").length-1 && $(this).find(".spacer").length <= 0){
